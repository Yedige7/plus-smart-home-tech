    ...
    public Optional<SensorsSnapshotAvro> updateState(SensorEventAvro event) {
        ...
        if (oldState != null) {
            if (oldState.getTimestamp().isAfter(eventTs)) {
                log.info("IGNORE older: hub={}, sensor={}, oldTs={}, evTs={}", hubId, sensorId, oldState.getTimestamp(), eventTs);
                return Optional.empty();
            }
            if (oldState.getData() != null && oldState.getData().equals(event.getPayload())) {
                log.info("IGNORE duplicate: hub={}, sensor={}, ts={}", hubId, sensorId, eventTs);
                return Optional.empty();
            }
        }
